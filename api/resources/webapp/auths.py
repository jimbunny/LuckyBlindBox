...
class  AuthorizationResource(Resource):
    ...
    def post(self, username="", password="", type="user"):
        ...
        """
        if type == "user":
            userInfo = MallUsersModel.query.filter_by(username=username).first()
            if (userInfo is None):
            ...
            #     return pretty_result(code.ERROR, data='', msg='请验证邮箱')
                ...
                if (MallUsersModel.check_password(MallUsersModel, userInfo.password, password)):
                    login_time = int(time.time())
                    userInfo.login_time = login_time
                    MallUsersModel.update(MallUsersModel)
                    user_id = userInfo.id
            ...
            userInfo = MallUsersModel.query.filter_by(facebook=username).first()
            login_time = int(time.time())
            userInfo.login_time = login_time
            MallUsersModel.update(MallUsersModel)
            user_id = userInfo.id
        ...
        if (auth_header):
            ...
            if (not auth_tokenArr or auth_tokenArr[0] != 'JWT' or len(auth_tokenArr) != 2):
                ...
                if not isinstance(payload, str):
                    user = MallUsersModel.get(MallUsersModel, payload['data']['id'])
                    if (user is None):
                        msg = '找不到该用户信息'
                    else:
